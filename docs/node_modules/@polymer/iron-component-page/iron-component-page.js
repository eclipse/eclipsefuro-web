...
`,is:"iron-component-page",properties:{/**
     * URL of the Polymer Analyzer descriptor to fetch and display.
     */descriptorUrl:{type:String,value:"analysis.json",observer:"_descriptorUrlChanged"},/**
     ...
     */demoSrcPrefix:String,_loading:Boolean,_descriptorError:Object,_descriptor:{type:Object,observer:"_descriptorChanged"},_path:String,_narrow:Boolean,_title:{type:String,observer:"_titleChanged"}},_onViewChanged(){this.$.viewer.scrollIntoView()},_onNavSelect(){// Note we need to listen for this event, and can't rely just on the
// path changing, because the user might click on the nav item they
// are already viewing.
this.$.viewer.scrollIntoView();if(this._narrow){this.$.drawer.close()}},_descriptorUrlChanged(){this._descriptorError=null},_descriptorChanged(descriptor){if(!descriptor||this._changing){return}this._changing=!0;this._descriptor=_flatten(descriptor);this._changing=!1},_titleChanged(title){window.document.title=title}});function _flatten(descriptor,flat){if(!flat){flat={namespaces:[],elements:[],metadata:{polymer:{behaviors:[]}},mixins:[],classes:[]}}for(var i=0;i<(descriptor.namespaces||[]).length;i++){_flatten(descriptor.namespaces[i],flat);flat.namespaces.push(descriptor.namespaces[i])}for(var i=0;i<(descriptor.classes||[]).length;i++){flat.classes.push(descriptor.classes[i])}for(var i=0;i<(descriptor.elements||[]).length;i++){flat.elements.push(descriptor.elements[i])}for(var descriptorBehaviors=((descriptor.metadata||{}).polymer||{}).behaviors,i=0;i<(descriptorBehaviors||[]).length;i++){flat.metadata.polymer.behaviors.push(descriptorBehaviors[i])}for(var i=0;i<(descriptor.mixins||[]).length;i++){flat.mixins.push(descriptor.mixins[i])}return flat}});