 ...
 */
export class ValidatorGoogleProtobufInt64Value {
  /**
   *
   * @param field
    ...
    return new Promise((resolve, reject) => {
      ...
      for (const constraintName in field._constraints) {
        ...
        switch (constraintName.toLowerCase()) {
           ...
           */
          case 'min':
            if (val !== undefined && val < parseInt(constraint.is, 10)) {
              const NODE = {};
              NODE.message = constraint.message;
              NODE.name = constraintName;
              NODE.node = field;
          ...
          case 'step':
            ...
            let modulo = parseInt(constraint.is, 10);
            // eslint-disable-next-line no-case-declarations
            let valueToProove = parseInt(val, 10);
            // eslint-disable-next-line no-case-declarations
            let f = 1; // factor
