...
describe('furo-entity-agent fieldmask', () => {
  ...
  it('update method with patch should provide a field_mask', (done) => {
    ...
    entityAgent.addEventListener("save-success", (r) => {
      ...
      assert.equal(JSON.stringify(r.detail.json.update_mask), '["paths: description","paths: cost_limit.currency_code"]');
      // eslint-disable-next-line no-prototype-builtins
  ...
  it('should create a fieldmask with complex types', () => {
    ...
    assert.equal(JSON.stringify(result), '["paths: data.id","paths: data.cost_limit.currency_code","paths: data.cost_limit.display_name","paths: data.cost_limit.nanos","paths: data.cost_limit.units","paths: data.description","paths: data.display_name","paths: data.end.day","paths: data.end.display_name","paths: data.end.month","paths: data.end.year","paths: data.members","paths: data.start.day","paths: data.start.display_name","paths: data.start.month","paths: data.start.year"]');
