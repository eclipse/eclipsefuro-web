...
class DemoFuroFilter extends FBP(LitElement) {
  ...
  static get styles() {
    // language=CSS
    return (
      Theme.getThemeForComponent('DemoFuroFilter') ||
      css`
        :host {
          display: block;
          height: 100%;
    ...
    return html`
      <furo-vertical-flex>
        <div>
          ...
          <p>
            We use a singleton resource to deliver possible filter options. Here we use
            /projects/filter
          </p>
        </div>
            ...
            <div style="width: 75%; margin: 0 auto;">
              ...
              <furo-horizontal-flex>
                <furo-data-text-input
                  ...
                  leading-icon="search"
                ></furo-data-text-input>
                <furo-button label="clear filter" @-click="--filterCleared"></furo-button>
              </furo-horizontal-flex>

              <demo-project-filter-form
                ƒ-bind-data="--entity"
                @-filter-changed="--filterChanged"
                @-filter-cleared="--filterChanged"
              ></demo-project-filter-form>

              <p>Filter Array</p>
              <furo-pretty-json
                ƒ-inject-data="--filterChanged, --filterCleared(*.dummy)"
              ></furo-pretty-json>
            </div>
