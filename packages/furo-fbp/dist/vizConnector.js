...
function messages(root) {
    ...
    window.onmessage = m => {
        ...
        if (m.data.type === 'PARENT_COMPONENT_REQUEST') {
            ...
            while (!stack[stack.length - 1].endsWith('::shadow') &&
                stack.length > 1) {
                stack.pop();
            }
        ...
        if (m.data.type === 'BREAKPOINTS') {
            ...
            sessionStorage.setItem('__fbp_breakpoints', JSON.stringify(m.data.breakpoints));
        }
...
//# sourceMappingURL=vizConnector.js.map