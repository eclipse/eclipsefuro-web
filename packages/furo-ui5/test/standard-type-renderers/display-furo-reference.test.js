...
describe('display-furo-reference', () => {
  ...
  it('should show template according to the value of the data', done => {
    dao.injectRaw({ display_name: 'display name', id: 'id' });
    setTimeout(() => {
      assert.equal(display._field.display_name._value, 'display name');
      assert.equal(display._displayValue, 'display name');
      done();
    }, 100);
  });
