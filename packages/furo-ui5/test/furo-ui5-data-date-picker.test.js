...
describe('furo-ui5-data-date-picker', () => {
  ...
  it('should apply meta and constraints to the bound field (google.type.Date)', done => {
    ...
    setTimeout(()=>{
      assert.equal(datepicker._state.disabled, false, 'check disabled');
      assert.equal(datepicker._state.placeholder, 'google date input label via meta', 'check placeholder');
      assert.equal(datepicker._state.readonly, false, 'check readonly');
      assert.equal(datepicker._state.value, '31.12.20', 'check value');
      assert.equal(datepicker._state.valueState, 'None', 'check valueState');
      assert.equal(datepicker._state.minDate, '2020-07-30', 'check if set minDate from response meta');
      assert.equal(datepicker._state.maxDate, '2020-09-30', 'check if set maxDate from response meta');
  ...
  it('should apply meta and constraints to the bound field (type string)', done => {
    ...
    setTimeout(()=>{
      assert.equal(datepicker2._state.disabled, false, 'check disabled');
      assert.equal(datepicker2._state.placeholder, 'date input label via meta', 'check placeholder');
      assert.equal(datepicker2._state.readonly, false, 'check readonly');
      ...
      assert.equal(datepicker2._state.maxDate, '2099-12-31', 'check if set MaxDate from response meta');
      assert.equal(datepicker2._state.minDate, '1800-01-01', 'check if set minDate from response meta');
