...
export class CellFuroPropertyRepeated extends FBP(LitElement) {
  ...
  static get styles() {
    // language=CSS
    return (
      Theme.getThemeForComponent('CellFuroPropertyRepeated') ||
      css`
        :host {
          display: none;
        }
  ...
  _updateFieldList() {
    ...
    if (this._field.repeats && this._field.repeats.length) {
      this._field.repeats.forEach(item => {
        const renderName = `cell-${item.data['@type']._value
          .replace(/.*\//, '')
          .replaceAll('.', '-')
          .toLocaleLowerCase()}`;
...
window.customElements.define('cell-furo-property-repeated', CellFuroPropertyRepeated);
