...
describe('furo-data-text-input', () => {
  let dataTextInput, host, dataObject, secondTextInput, invalidBoundTextInput, deeplink, entityAgent;
  ...
  beforeEach(async () => {
    ...
    host = testbind._host;
    [, dataTextInput,secondTextInput,dataObject,deeplink,entityAgent,invalidBoundTextInput] = testbind.parentNode.children;
  ...
  it('should receive value with bind', (done) => {
    ...
    host._FBPAddWireHook("--hts", ( ) => {
      dataObject.addEventListener("data-changed", ( ) => {
  ...
  it('should listen field-became-invalid event add set error', (done) => {
      ...
      let err = {"description": "minimal 3 charaters", constraint: "min"};
    setTimeout(() => {
      dataTextInput.field.addEventListener("field-became-invalid",()=>{

        setTimeout(() => {
          assert.equal(dataTextInput.error, true);
          assert.equal(dataTextInput._theInputElement.getAttribute("errortext"), "minimal 3 charaters");
          done();
        }, 10)
