...
export class CollectionDropdownHelper {
    ...
    caller._pseudoDropdownList = [
      {
        ...
        label: caller._fieldDisplayNodeToUpdate ? caller._fieldDisplayNodeToUpdate._value: caller._fieldNodeToUpdate._value,
        selected: true,
      },
    ];
  ...
  static  mapDataToList(caller, list) {
    ...
    if (!caller._fieldNodeToUpdate || !caller._fieldNodeToUpdate._value) {
      ...
      for (let i = 0; i < list.length; i += 1) {
        ...
        if (caller._fieldNodeToUpdate._value === list[i][this.valueField]) {
          item.selected = true;
          isSelected = true;
        }
  ...
  static setOptionItems(caller) {
    if (
      ...
      caller.addItems(arr);
      const selectedObj = caller._dropdownList.find(obj => obj.id === caller._fieldNodeToUpdate._value);
  ...
  static bindData(caller, fieldNode) {
    ...
    if (caller.binder.fieldNode) {
      ...
      if (caller.valueSubField) {
        caller._fieldNodeToUpdate = this.getValueByPath(caller.binder.fieldNode, caller.valueSubField);
