...
class DemoFuroSelectInput extends FBP(LitElement) {
  ...
  static get styles() {
    // language=CSS
    return Theme.getThemeForComponent('DemoFuroSelectInput') || css`
        :host {
            display: block;
            height: 100%;
  ...
  _FBPReady(){
    ...
    // get the snippet
    let demo = this.shadowRoot.querySelector("furo-demo-snippet");

    setTimeout(()=>{
      let l = demo.shadowRoot.querySelectorAll("furo-select-input");
