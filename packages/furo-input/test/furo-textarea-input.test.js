...
describe('furo-textarea-input', () => {
  ...
  beforeEach(async () => {
    const testbind = await fixture(html`
      <test-bind>
        <template>
          <furo-textarea-input min="1" max="100"></furo-textarea-input>
        </template>
      </test-bind>
    `);
    ...
    inputField = element.shadowRoot.querySelector('textarea');
  });

  it('should be a furo-textarea-input', done => {
    // keep this test on top, so you can recognize a wrong asignment
    assert.equal(element.nodeName.toLowerCase(), 'furo-textarea-input');
    done();
  });
  ...
  it('should clear the errorstate and resume to the initial errortextarea', done => {
    ...
    element.setError('Textarea from outside');
    element.clearError();
  ...
  it('instantiating furo-textarea-input with default properties works', () => {
    assert.equal(element.tagName.toLowerCase(), 'furo-textarea-input');
  });
});
