...
describe('furo-date-input', () => {
  ...
  it('should be a furo-date-input', done => {
    // keep this test on top, so you can recognize a wrong asignment
    assert.equal(element.nodeName.toLowerCase(), 'furo-date-input');
    done();
  });
  ...
  it('should  accept date value with setValue', done => {
    element.setValue('2021-12-10');
    assert.equal(inputField.value, '2021-12-10');
    assert.equal(element.value, '2021-12-10');
    done();
  });
  ...
  it('instantiating furo-date-input with default properties works', () => {
    assert.equal(element.tagName.toLowerCase(), 'furo-date-input');
  });
});
