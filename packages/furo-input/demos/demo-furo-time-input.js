...
class DemoFuroTimeInput extends FBP(LitElement) {
  ...
  static get styles() {
    // language=CSS
    return (
      Theme.getThemeForComponent('DemoFuroTimeInput') ||
      css`
        :host {
          display: block;
          height: 100%;
    ...
    return html`
      <furo-vertical-flex>
        <div>
          ...
          <p>
            If you type in a time outside the min max range or the step, an "error" will be
            indicated. But not the error text.
          </p>
          <p>
            You can also type in a time in a "furo-input-field" (the 3rd field). But when you feed
            "furo-time-input" with bad data, it displays "--:--"
          </p>
        </div>
