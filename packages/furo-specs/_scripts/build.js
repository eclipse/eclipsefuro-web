...
if (speclist.enums) {
  speclist.enums.forEach((filename) => {
    ...
    // create if not exist
    if (!Enumlist[target]) {
      Enumlist[target] = { 'imports': new Set, enums: [], 'options': {}, package: spec.__proto.package };
    }
    if (spec.__proto.imports) {
      spec.__proto.imports.forEach((i) => {
        Enumlist[target].imports.add(i);
      });
    }

    if (spec.__proto.options) {
      for (let key in spec.__proto.options) {
        Enumlist[target].options[key] = spec.__proto.options[key];
      }
    }


    spec.values = Object.keys(spec.values).map(function(key) {
      return {"name": key, "value":spec.values[key]};
    });



    Enumlist[target].enums.push(spec);
  });
}
for (let target in Enumlist) {
  // make array from import set
  let type = Enumlist[target];
  ...
  sh(pathToSimpleGeneratorBinary + 'simple-generator', ['-d', jsonfilename, '-t', TPLDirSingle + '/enum.proto.tmpl', '>', BuildDir + '/protos/' + target]);
}
